...
import { getJSXElementForSourceCodePosition, getJSXASTByLookupId } from "../utils/ast-parsers";
...
export const EditorPane: FunctionComponent<Props> = ({ codeEntries, onCodeChange, selectedElementId, onSelectElement }) => {
  ...
  useEffect(() => {
    ...
    if (selectedElementId && selectedElementId.codeId === activeCodeId) {
      ...
      let path;
      try {
        path = getJSXASTByLookupId(parseAST(activeCode), selectedElementId.lookUpId);
      } catch (err) {}
  ...
  }, [activeCode, activeCodeId, selectedElementId?.codeId, selectedElementId?.lookUpId]);
  ...
  useEffect(() => {
    return editorRef.current?.editor?.onDidChangeCursorPosition((e) => {
      ...
      if (lookUpId !== undefined && (activeCodeId !== selectedElementId?.codeId || lookUpId !== selectedElementId?.lookUpId)) {
  ...
  return <Tabs activeTab={activeTab} onChange={setActiveTab} tabs={codeEntries.map(entry => ({
    name: entry.filePath,
    render: () => (
      <MonacoEditor
        ref={editorRef}
