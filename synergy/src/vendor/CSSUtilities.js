...
(function () {
  ...
  var busyinit = false,
    ...
    FATAL_ERROR_INVALID_MODE =
      FATAL_ERROR_PREFIX + " The specified mode is not valid",
    FATAL_ERROR_INVALID_ASYNC =
      FATAL_ERROR_PREFIX + " The specified async setting is not valid",
    FATAL_ERROR_NOT_DOCUMENT =
      FATAL_ERROR_PREFIX + " The specified document is not a Document",
    FATAL_ERROR_NOT_ABSOLUTE_URI =
      FATAL_ERROR_PREFIX + " The specified base is not an absolute URL",
    FATAL_ERROR_INVALID_WATCH =
      FATAL_ERROR_PREFIX + " The specified watch setting is not valid",
    FATAL_ERROR_INVALID_ATTRS =
      FATAL_ERROR_PREFIX + " The specified attributes setting is not valid",
    FATAL_ERROR_NOT_SAPI =
      FATAL_ERROR_PREFIX + " The specified api settings are not valid",
    FATAL_ERROR_INVALID_SAPI =
      FATAL_ERROR_PREFIX +
      " Your Selectors API is not returning the right data",
    FATAL_ERROR_MISSING_SAPI =
      FATAL_ERROR_PREFIX + " The Selectors API is missing",
    FATAL_ERROR_NO_XHR =
      FATAL_ERROR_PREFIX + " Unable to communicate with the network",
    //-- error message variables (for console messages) --//
    errorMessageNoElement =
      "CSSUtilities.%method has an invalid Element reference or ID",
    errorMessageNoSelector =
      "CSSUtilities.%method requires a valid Selector reference",
    errorMessageMultipleSelector =
      "CSSUtilities.%method can only process one Selector at a time",
    errorMessageInvalidSSID =
      "CSSUtilities.%method has an invalid Stylesheet ID",
    errorMessageNotAfterAPI =
      FATAL_ERROR_PREFIX +
      ' You cannot define "%var" after "api",' +
    ...
    MEDIA_TYPES_LIST =
      "aural,braille,embossed,handheld,print,projection,reader,screen,speech,tty,tv",
    ...
    REGEX_CLASS_SELECTORS = /(\.[_a-z]+[_a-z0-9-:\\]*)/gi,
  ...
  this.define = function (varname, value1, value2) {
    ...
    switch (varname) {
      ...
      case "mode":
        ...
        if (
          typeof value1 != TYPE_STRING ||
          !/^(author|browser)$/i.test(value1)
        ) {
      ...
      case "watch":
        ...
        if (
          !(
            typeof value1 == TYPE_BOOLEAN ||
            (mode == MODE_AUTHOR && value1 == NULL_VALUE)
          )
        ) {
          throw new Error(FATAL_ERROR_INVALID_WATCH);
        }
      ...
      case "api":
        ...
        if (typeof value2 == TYPE_FUNCTION) {
          ...
          if (
            typeof nodes != TYPE_OBJECT ||
            nodes == NULL_VALUE ||
            typeof nodes.length == TYPE_UNDEFINED
          ) {
  ...
  this.init = function (callback) {
    ...
    api =
      typeof page.querySelectorAll != TYPE_UNDEFINED && api !== BOOLEAN_TRUE
        ? BOOLEAN_FALSE
        : BOOLEAN_TRUE;
    ...
    this._isXML =
      (OPERA && page.documentElement.namespaceURI != NULL_VALUE) ||
      (!OPERA &&
        typeof page.xmlVersion != TYPE_UNDEFINED &&
  ...
  this.getCSSStyleSheetRules = function () {
    ...
    if (
      typeof args.accept.properties != TYPE_UNDEFINED &&
      typeof args.accept.css == TYPE_UNDEFINED
    ) {
      args.accept.css = "";
    ...
    if (
      args.ssid !== -1 &&
      arrayContains(this._stylesheets, args.ssid, "ssid") == NULL_VALUE
    ) {
      throw new Error(
        errorMessageInvalidSSID.replace("%method", "getCSSStyleSheetRules")
      );
    }
    ...
    return dispatchReturn(args.onfinished, function () {
      ...
      for (var i = 0; i < THIS._cssRules.length; i++) {
        ...
        if (mediaMatches(args.media, THIS._cssRules[i])) {
          ...
          if (args.ssid !== -1 && THIS._cssRules[i].ssid !== args.ssid) {
            continue;
          }
          ...
          for (var j in THIS._cssRules[i]) {
            if (!THIS._cssRules[i].hasOwnProperty(j)) {
              continue;
            }
          ...
          if (
            args.accept == "*" ||
            typeof args.accept.index != TYPE_UNDEFINED
          ) {
            cssrule.index = i;
          }
    ...
    return dispatchReturn(args.onfinished, function () {
      ...
      for (var i = 0; i < rules.length; i++) {
        for (var j in rules[i].properties) {
          if (
            !rules[i].properties.hasOwnProperty(j) ||
      ...
      if (args.directonly == BOOLEAN_FALSE) {
        ...
        for (var i = 0; i < allselectors.length; i++) {
          ...
          for (var j = 0; j < ancestors.length; j++) {
            var nodes = getElementsBySelector(
              allselectors[i].replace(REGEX_PSEUDO_CLASSES_EXCEPT_NOT, "")
            );
            if (nodes.length > 0) {
  ...
  this.getCSSSelectorSpecificity = function () {
    ...
    if (
      typeof args.selector != TYPE_STRING ||
      args.selector == NULL_VALUE ||
      trim(args.selector) == ""
    ) {
      throw new Error(
        errorMessageNoSelector.replace("%method", "getCSSSelectorSpecificity")
      );
    }
    ...
    //if selector is a string containing any commas, throw the multiple selector error
      ...
      throw new Error(
        errorMessageMultipleSelector.replace(
          "%method",
    ...
    if (args.element != NULL_VALUE) {
      args.element = processElementArgument(
        args.element,
        "getCSSSelectorSpecificity"
      );
    }
      ...
      if (args.element != NULL_VALUE) {
        var applies = BOOLEAN_FALSE,
          nodes = getElementsBySelector(
            args.selector.replace(REGEX_PSEUDO_CLASSES_EXCEPT_NOT, "")
          );
        if (nodes.length > 0) {
          for (var j = 0; j < nodes.length; j++) {
            if (nodes[j] == args.element) {
  ...
  function createAuthorRulesData(oncomplete) {
    ...
    function continuation() {
      ...
      function stage1_checkStyleSheetForDataWrapper(cdcount) {
        ...
        stage2_checkStyleSheetForData(
          THIS._cssdata[cdcount],
        ...
        //containing the text of a successful request, or an error message
          ...
          requestStyleSheet(cssdata.href, function (dataobject) {
            ...
            if (dataobject.message != NULL_VALUE) {
              ...
              if (
                cssdata.message == ERROR_NOT_CSS ||
                /^[1-9]{1,3}/.test(cssdata.message)
              ) {
      ...
      function stage3_checkStyleSheetForImportsWrapper() {
        ...
        function stage4_checkStyleSheetForImports(cdindex) {
          ...
          //else proceed
            ...
            var importsheets = extractImportStatements(
              THIS._cssdata[cdindex].text
            );
      ...
      function stage6_checkImportForData(
        cdcount,
        impcount,
        importobj,
        oncomplete
        ...
        if (
          arrayContains(THIS._cssdata, importobj.href, "href") != NULL_VALUE
        ) {
      ...
      function stage7_processStyleSheetsData() {
        ...
        for (var i = 0; i < THIS._cssdata.length; i++) {
          ...
          if (THIS._cssdata[i].href) {
            var arycontains = arrayContains(
              THIS._stylesheets,
              THIS._cssdata[i].href,
              "href"
            );
            if (
              arycontains != NULL_VALUE &&
              arycontains.message != MESSAGE_DUPLICATE
            ) {
        ...
        THIS._stylesheets.sort(function (a, b) {
  ...
  function getStyleSheetNodes() {
    ...
    for (var i = 0; i < kids.length; i++) {
      if (
        kids[i].nodeType == 7 &&
        kids[i].target.toLowerCase() == "xml-stylesheet"
      ) {
    ...
    for (var i = 0; i < allnodes.length; i++) {
      if (
        (/link/i.test(allnodes[i].nodeName) &&
  ...
  function processStyleNode(node, owner) {
    ...
    if (/xml\-stylesheet/.test(owner)) {
      ...
      for (var i = 0; i < nodedata.length; i++) {
        ...
        pseudoattrs[nodedata[i][0]] = nodedata[i][1].substr(
          1,
          nodedata[i][1].length - 2
        );
      }
    }
    ...
    //just in case any unknown devices have the same object reference
      ...
      var disnode =
          node[
            typeof node.styleSheet != TYPE_UNDEFINED ? "styleSheet" : "sheet"
          ],
    ...
    //otherwise it will be a style element
      ...
      if (!isdisabled || watch === NULL_VALUE) {
        ...
        //for any other browser
          ...
          for (
            var nodedata = "", kids = node.childNodes, i = 0;
            i < kids.length;
            i++
          ) {
            switch (kids[i].nodeType) {
              case 3:
                nodedata += kids[i].nodeValue;
                break;
              case 4:
                nodedata += "<![CDATA[" + kids[i].nodeValue + "]]>";
                break;
              case 8:
                nodedata += "<!--" + kids[i].nodeValue + "-->";
                break;
    ...
    if (/xml\-stylesheet/.test(owner)) {
      cssdata.media =
        typeof pseudoattrs.media != TYPE_UNDEFINED
          ? pseudoattrs.media
          : MEDIA_ALL;
    } else {
  ...
  function extractImportStatements(styletext) {
    ...
    if (matches) {
      for (var i = 0; i < matches.length; i++) {
        ...
        var media = trim(
          trim(matches[i].replace(importreg, "$2")).replace(/^\)/, "")
        );
  ...
  function strainStyleSheet(styletext, importsheets) {
    ...
    styletext = styletext.replace(/@(charset|namespace)[^;]+;/gim, "");
    ...
    if (typeof importsheets != TYPE_UNDEFINED && importsheets == BOOLEAN_TRUE) {
      styletext = styletext.replace(/@import[^;]+;/gim, "");
    }
    ...
    //so any which come after a block of normal CSS don't apply and get deleted
      ...
      var cssdata = styletext
        .replace(/(@import[^;]+;)/gim, "{SPLIT}$1{SPLIT}")
        .split("{SPLIT}");
      ...
      for (
        var styletext = "", nomoreimports = NULL_VALUE, i = 0;
        i < cssdata.length;
        i++
      ) {
        ...
        if (/@import[^;]+;/i.test(cssdata[i])) {
          ...
          if (nomoreimports === NULL_VALUE) {
            nomoreimports = BOOLEAN_FALSE;
          }

          //if they're still valid,  add it to the compiled text
          if (nomoreimports === BOOLEAN_FALSE) {
            styletext += cssdata[i];
          }
        }
    ...
    styletext = styletext.replace(/@(font\-face|page)[^\}]+\}/gim, "");
  ...
  function parseStyleSheetCSSText(cssdata) {
    ...
    var gencontent = cssdata.text.match(/(content:[^;]+;)/gim);
    if (gencontent) {
    ...
    if (gencontent) {
      for (var g = 0; g < gencontent.length; g++) {
        for (var i = 0; i < cssdata.text.length; i++) {
          for (var j = 0; j < cssdata.text[i].length; j++) {
            cssdata.text[i][j] = cssdata.text[i][j].replace(
              "[G" + g + "]",
              gencontent[g]
            );
          }
    ...
    for (var i = 0; i < cssdata.text.length; i++) {
      ...
      //if we have three then the first is an opening @media declaration
        ...
        currentmedia = trim(
          cssdata.text[i][0].replace("@media", "")
        ).toLowerCase();
        currentowner = "@media";
  ...
  function createBrowserRulesData(oncomplete) {
    ...
    for (var i = 0; i < stylesheets.length; i++) {
      ...
      if (typeof stylesheets[i].ownerNode != TYPE_UNDEFINED) {
        ...
        if (
          SAFARI3 &&
          /xml\-stylesheet/.test(stylesheets[i].ownerNode.nodeName)
        ) {
        ...
        //otherwise try to parse the stylesheet as normal
          ...
          try {
            parseStyleSheet(
              ...
              stylesheets[i].media.length > 0
                ? trim(stylesheets[i].media.mediaText)
                : MEDIA_ALL
            );
          } catch (err) {
  ...
  function parseStyleSheet(sheet, owner, media) {
    ...
    try {
      ...
      if (!isdisabled) {
        ...
        for (
          var rules = sheet.cssRules, rulecount = 0, i = 0;
          i < rules.length;
          i++
        ) {
          if (rules.item(i).type == 1) {
            ...
            for (
              var subrules = rules.item(i).cssRules, j = 0;
              j < subrules.length;
              j++
            ) {
    ...
    for (i = 0; i < rules.length; i++) {
      ...
      if (rule.type == 3) {
        ...
        while (parent) {
          ...
          //the media object belongs to its ownerRule object instead
          try {
            var context = parent.media.mediaText;
          } catch (err) {
    ...
    for (var i = 0; i < rules.length; i++) {
      ...
      if (rule.type == 1) {
        ...
        for (var c = 0; c < styletext.length; c++) {
          ...
          while (styletext[c].length > 2) {
            styletext[c][1] += ":" + styletext[c].pop();
          }
          ...
          if (
            rule.style.getPropertyPriority(trim(styletext[c][0])) ==
              "important" &&
  ...
  function parseIEStyleSheet(sheet, owner, media, href) {
    ...
    try {
      ...
      if (!isdisabled) {
        ...
        var gencontent = styletext.match(/(content\s*:\s*[\'\"].*(;|$))/gim);
        if (gencontent) {
          for (var g = 0; g < gencontent.length; g++) {
        ...
        if (gencontent) {
          for (var g = 0; g < gencontent.length; g++) {
            for (var i = 0; i < styletext.length; i++) {
              for (var j = 0; j < styletext[i].length; j++) {
                styletext[i][j] = styletext[i][j].replace(
                  "[G" + g + "]",
                  gencontent[g]
                );
              }
        ...
        if (typeof href == TYPE_UNDEFINED) {
          href =
            sheet.href == NULL_VALUE || sheet.href == ""
              ? NULL_VALUE
              : qualifyHREF(sheet.href, base);
        }
      ...
      THIS._stylesheets.push({
        ssid: sheet.__ssid,
        href: sheet.href ? qualifyHREF(sheet.href, base) : NULL_VALUE,
        owner: owner,
        media: media,
        stylesheet: sheet,
      ...
      //otherwise, see if this selector matches any elements
        ...
        nodes = getElementsBySelector(
          altstates == BOOLEAN_TRUE
            ? THIS._cssRules[i].selector.replace(
                REGEX_PSEUDO_CLASSES_EXCEPT_NOT,
                ""
              )
      ...
      if (nodes.length > 0) {
        for (var j = 0; j < nodes.length; j++) {
          ...
          if (nodes[j] == element) {
            ...
            for (
              var optionalprops = ["media", "xmedia", "owner", "ssid", "href"],
                k = 0;
              k < optionalprops.length;
              k++
            ) {
              if (
                accept == "*" ||
                typeof accept[optionalprops[k]] != TYPE_UNDEFINED
              ) {
                //we need this condition because the "xmedia"
                //property is only available in author mode
                if (
                  typeof THIS._cssRules[i][optionalprops[k]] != TYPE_UNDEFINED
                ) {
                  ruleobj[optionalprops[k]] =
                    THIS._cssRules[i][optionalprops[k]];
                }
              }
            }
        ...
        //in order to find and mark the cancelled properties
          ...
          for (var j = 0; j < i; j++) {
            ...
            for (var jp in rules[j].allproperties) {
              if (
                !rules[j].allproperties.hasOwnProperty(jp) ||
              ...
              for (var ip in rules[i].allproperties) {
                if (
                  !rules[i].allproperties.hasOwnProperty(ip) ||
    ...
    if (dosort) {
      ...
      for (var i = 0; i < rules.length; i++) {
        ...
        for (var j in rules[i].allproperties) {
          if (
            !rules[i].allproperties.hasOwnProperty(j) ||
          ...
          if (IMPORTANT_RULE.test(rules[i].allproperties[j].value)) {
            ...
            for (var x = i + 1; x < rules.length; x++) {
              ...
              if (
                typeof rules[x].allproperties[j] != TYPE_UNDEFINED &&
            ...
            for (var x = i; x < rules.length; x++) {
              ...
              if (x == precedence) {
                ...
                for (var y = 0; y < x; y++) {
                  if (
                    typeof rules[y].allproperties[j] != TYPE_UNDEFINED &&
          ...
          if (typeof LONGHAND_PROPERTIES[j] != TYPE_UNDEFINED) {
            ...
            for (var x = 0; x <= i; x++) {
              ...
              for (var p = 0; p < LONGHAND_PROPERTIES[j].length; p++) {
                ...
                if (
                  typeof rules[x].allproperties[longhand] != TYPE_UNDEFINED &&
  ...
  function lowercaseIETypeSelectors(selectorstring) {
    ...
    var attrselectors = selectorstring.match(
      /(^|[^\(])(\[[^\]]+\])($|[^\)])/gi
    );
    if (attrselectors) {
      for (var a = 0; a < attrselectors.length; a++) {
        ...
        if (attrselectors[a].charAt(0) != "[") {
          attrselectors[a] = attrselectors[a].substr(
            1,
            attrselectors[a].length - 1
          );
        }
    ...
    var cidselectors = selectorstring.match(/([#\.][a-z]+[_a-z0-9-:\\]*)/gi);
    if (cidselectors) {
    ...
    selectorstring = selectorstring.replace(/([A-Z1-6]+)/g, function (a) {
  ...
  function parsePropertyText(styletext, properties, inherited) {
    ...
    for (var i = 0; i < styletext.length; i++) {
      ...
      if (
        !inherited ||
        (inherited && typeof INHERITED_PROPS[key] != TYPE_UNDEFINED)
      ) {
        ...
        //	by a content property with "!important" in its text)
        if (
          typeof properties[key] == TYPE_UNDEFINED ||
  ...
  function mediaMatches(media, rule) {
    ...
    for (var j = 0; j < media.length; j++) {
      ...
      if (
        typeof mediaTypes[media[j]] != TYPE_UNDEFINED ||
        (typeof mediaTypes[MEDIA_NONE] == TYPE_UNDEFINED &&
          media[j] == MEDIA_ALL) ||
        (typeof mediaTypes[MEDIA_ALL] != TYPE_UNDEFINED &&
          media[j] != MEDIA_NONE)
      ) {
  ...
  function getViewMedia() {
    ...
    try {
      ...
      var testmedianode = pagebody.insertBefore(
      ...
      if (IEXPLORER) {
        ...
        viewmediastylesheet.addRule(
          "#cssutilitiestest" + "medianode",
          "display:block !important;"
        );
      } else {
        //create a style node and add the rule as a child text node
        var viewmedianode = page
          .getElementsByTagName("head")
          .item(0)
          .appendChild(page.createElement("style"));
        viewmedianode.setAttribute("type", "text/css");
        viewmedianode.appendChild(
          page.createTextNode(
            "#cssutilitiestest" + "medianode{display:block !important;}"
          )
        );
      ...
      for (var i = 0; i < mediaTypes.length; i++) {
        ...
        if (IEXPLORER) {
          viewmediastylesheet.media = mediaTypes[i];
        } else {
          viewmediastylesheet.media.mediaText = mediaTypes[i];
        }
        ...
        if (
          (IEXPLORER && testmedianode.currentStyle.display == "block") ||
          (!IEXPLORER &&
            page.defaultView
              .getComputedStyle(testmedianode, "")
  ...
  function watchStyleSheets() {
    ...
    for (var livesheets = [], i = 0; i < THIS._stylesheets.length; i++) {
      ...
      var stylesheet =
        mode == MODE_AUTHOR
          ? THIS._stylesheets[i].stylenode == BOOLEAN_FALSE ||
            THIS._stylesheets[i].stylenode == NULL_VALUE
            ? NULL_VALUE
            : SAFARI || KONQUEROR || WEBKIT
            ? THIS._stylesheets[i].stylenode
            : //nb. although we know this condition is IE vs. others, an object test is better
              THIS._stylesheets[i].stylenode[
                typeof THIS._stylesheets[i].stylenode.styleSheet !=
                TYPE_UNDEFINED
                  ? "styleSheet"
                  : "sheet"
              ]
    ...
    if ((SAFARI || KONQUEROR || WEBKIT) && mode == MODE_BROWSER) {
      ...
      function createStyleSheetIdents() {
        for (
          var idents = [], dsheets = page.styleSheets, i = 0;
          i < dsheets.length;
          i++
        ) {
          if (typeof dsheets[i].ownerNode.__ident == TYPE_UNDEFINED) {
            dsheets[i].ownerNode.__ident =
      ...
      var controlidents = createStyleSheetIdents(),
        watcher = window.setInterval(function () {
          ...
          if (controlidents.join() != currentidents.join()) {
            ...
            THIS.init(
              typeof THIS.initcallback != TYPE_UNDEFINED
                ? THIS.initcallback
                : NULL_VALUE
            );
          }
  ...
  function getTheseElements(tagname) {
    ...
    if (tagname == "*") {
      for (var i = 0; i < thesenodes.length; i++) {
        if (
          thesenodes[i].nodeType != 1 ||
          thesenodes[i].tagName.charAt(0) == "/"
        ) {
  ...
  function processElementArgument(argelement, calledby) {
    ...
    if (typeof argelement == TYPE_STRING && argelement.charAt(0) == "#") {
      argelement = page.getElementById(
        argelement.substr(1, argelement.length - 1)
      );
    }

    //if element is undefined, null, or not an element, throw an error
    if (
      typeof argelement == TYPE_UNDEFINED ||
      argelement == NULL_VALUE ||
      typeof argelement.nodeType == TYPE_UNDEFINED ||
      argelement.nodeType != 1
    ) {
  ...
  function processMediaArgument(argmedia) {
    ...
    if (
      typeof argmedia == TYPE_UNDEFINED ||
      argmedia == "" ||
      argmedia == NULL_VALUE
    ) {
  ...
  function processAcceptArgument(argaccept) {
    ...
    if (
      typeof argaccept == TYPE_UNDEFINED ||
      argaccept == "" ||
      argaccept == NULL_VALUE ||
      argaccept == "null"
    ) {
      argaccept = "*";
    }
  ...
  function ajaxload(async, uri, oncomplete, onfail) {
    ...
    if (
      requestobject == NULL_VALUE &&
      typeof window.XMLHttpRequest != TYPE_UNDEFINED
    ) {
      try {
    ...
    try {
      requestobject.setRequestHeader("User-Agent", LIBRARY_VERSION_STRING);
    } catch (err) {}
